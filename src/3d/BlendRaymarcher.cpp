...
namespace _3D
{
	BlendRaymarcher::BlendRaymarcher()
		: Raymarcher3D(), mCurrentBlend(Union), mSmooth(false), mBlendFactor(1.f), mMove(true), mDistance(2.f), mRatio(0.f), mBackgroundColor(1.f)
	{
		mShader.reset(Shader::FromGLSLTextFiles("res/vertex.glsl", "res/3d/blend.glsl"));
		InitShader();
	}
